    ...
    <ResponseContext.Consumer>
      {(context: IResponseContext) =>
        ...
        context.response.results.map((result: any) => (
          <ShowTile key={result.id} tmdbId={result.id} name={result.original_name} genreIds={result.genre_ids} firstAirDate={result.first_air_date} overview={result.overview} poster={result.poster_path} />
        ))
      }
    </ResponseContext.Consumer>
  );
